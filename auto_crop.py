...
def crop_face(imgpath):
    ...
    if len(rects) is None:
        return print("Sorry. HOG could not detect any faces from your image.")
    for (i, rect) in enumerate(rects):
        (x, y, w, h) = face_utils.rect_to_bb(rect)
        
        top, bottom, left, right = crop_boundary(y, y + h, x, x + w, len(rects) <= 2)
        crop_img_path = f"{imgpath.split('.')[0]}_crop_{i}.png"
        crop_img = frame[top: bottom, left: right]
        cv2.imwrite(crop_img_path, cv2.cvtColor(crop_img, cv2.COLOR_RGB2BGR))

    return print(f"SUCCESS: [{imgpath}]")
